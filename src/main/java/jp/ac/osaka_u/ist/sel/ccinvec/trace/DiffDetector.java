...
import jp.ac.osaka_u.ist.sel.ccinvec.Def;
	...
	private static boolean executeDiff_test(ArrayList<SourceFile> fileList, Config config) {
		try{
			...
			if (File.separatorChar == '\\') {
				 ...
				 * */
				String[] cmdArray = { Paths.get(Def.CCINVEC_PATH, Def.DIFF_PATH).toString(),"-r" ,"-w","-I=\"\\t\"" ,config.getOldTarget(), config.getNewTarget() };
	...
	private static boolean executeDiff(SourceFile file) {
		try{
			...
			if (File.separatorChar == '\\') {
				...
				String[] cmdArray = { Paths.get(Def.CCINVEC_PATH, Def.DIFF_PATH).toString(), file.getOldPath(),
						file.getNewPath() };
				//	System.out.println(Arrays.asList(cmdArray));
				//System.out.println("kita  ====");
				pb = new ProcessBuilder(cmdArray);
