			...
			while((line = reader.readLine()) != null) {
				...
				if(line.length() > 0 && line.charAt(0) != '%') {
					...
					if(line.contains("BRANCH:")) {
						inputFlag = 0;
						config.setOldDir(removeSpace(line.replace("BRANCH:","")));
					}
...
//						inputFlag = 0;
//					}
					//git　or local project
					if(line.contains("TARGET:")) {
						if(removeSpace(line.replace("TARGET:","")).equals("git")) {
							config.setTargetGit(true);
						} else if(removeSpace(line.replace("TARGET:","")).equals("local")) {
							config.setTargetGit(false);
						}
						inputFlag = 0;
					}
